...
## F12 — H3 first-frame primary reference (correction pass)

The option is durable, global, experimental and default OFF. When ON, Picture1 is the same `119:0` IMAGE link as `node129.first_frame`, and user references retain exact order through their existing IMAGE branches as Pictures2..7; no filename is ever sent directly to an H3 `ref_image_*` socket and the local option is never sent as a node-129 control. Native `also_ref_first_frame` conflicts fail closed. The correction focal command was `$env:PYTHONPATH='src'; $env:TEMP=$env:TMP=$env:TMPDIR='C:\Codex\Orquestador-Test-Temp'; python -B -m unittest -v tests.test_f11_rebind_first_frame tests.test_f11_1b_capabilities` — **15 tests OK**. It covers OFF 0/1/6, ON 0/1/3/6, chunk2+ rebind, the former six-reference blocker, reduced-graph fail-closed topology, strict local flag/conflict checks, capabilities, and unresolved markers. Relevant regression command was `$env:PYTHONPATH='src'; $env:TEMP=$env:TMP=$env:TMPDIR='C:\Codex\Orquestador-Test-Temp'; $env:ORQ_TEST_TMP='C:\Codex\Orquestador-Test-Temp'; python -B -m unittest tests.test_f11_1a_generation_config tests.test_f11_1b_gui_preparation tests.test_f11_1b_capabilities tests.test_f7_chain_execution tests.test_recovery tests.test_f2_recovery_restart_e2e` — **115 tests OK**. No ComfyUI or real generation is part of this pass.
