...
class MainWindow(QMainWindow):
    def __init__(self, facade, project_root=None):
        ...
        self.initial_button.clicked.connect(self._choose_initial); self.chunk_count.currentTextChanged.connect(lambda _ : (self._invalidate(), self._sync_prompt_visibility())); [w.textChanged.connect(self._invalidate) for w in (self.project,self.execution,self.initial)]; self.initial.textChanged.connect(self._update_initial_preview); [w.textChanged.connect(self._invalidate) for w in self.prompts]; [w.valueChanged.connect(self._invalidate) for w in (self.megapixels,self.length,self.steps,self.fps)]; self.references.itemChanged.connect(lambda item: (self._update_reference_labels(), self._invalidate())); self.preflight.clicked.connect(self._preflight); self.prepare.clicked.connect(self._prepare); self.start.clicked.connect(self._start_chain); self.resume.clicked.connect(lambda:self._run(lambda:self.facade.resume_execution(self.project.text().strip(), self.execution.text().strip()))); self.retry.clicked.connect(lambda:self._run(lambda:self.facade.retry_execution(self.project.text().strip(), self.execution.text().strip()))); self.cancel.clicked.connect(lambda:self._run(lambda:self.facade.cancel_pending(self.project.text().strip(), self.execution.text().strip()))); self.assemble.clicked.connect(self._assemble); self._sync_prompt_visibility(); self.start.setEnabled(False)
        self.refresh()
