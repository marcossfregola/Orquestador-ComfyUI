...
class SubmitBoundary:
    ...
    def __init__(self, submitter, bridge=None):
    ...
    def submit(self, project, execution, chunk_id, prompt, **kwargs):
        ...
        attempt = chunk.new_attempt(); self.submitter.repository.save(project, [execution])
        try:
            ...
            self.submitter.repository.save(project, [execution])
        ...
        try:
            (self.bridge or ComfyUIJobBridge(self.submitter.repository)).bind(project, execution, chunk_id, str(attempt.id), ref, execution_id=execution.id)
