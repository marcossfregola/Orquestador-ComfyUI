...
class F6RecoverExecutionTests(unittest.TestCase):
    ...
    def test_G_completed_valid_path_delegates_completion_without_submit(self):
        with self.tempdir() as d:
            ...
            coordinator = ChunkExecutionCoordinator(repo, submitter, history, extractor=extractor, trusted_root=root, comfyui_output_root=root, correlator=lambda *_: corr, physical_validator=lambda *_: phys)
            self.assertEqual(ResumeExecutionUseCase(repo, backend, coordinator, submitter, output_root=root).resume(p.id, e.id).outcome, RecoveryOutcome.COMPLETE); submitter.submit.assert_not_called(); repo.close()
    ...
    def test_I_failed_retry_budget_is_explicitly_single_attempt(self):
        with self.tempdir() as d:
            ...
            backend.observe.side_effect=observe; submit=SubmitAttemptUseCase(repo,client); r=ResumeExecutionUseCase(repo,backend,Mock(),submit, output_root=root).resume(p.id,e.id,prompt={}); self.assertEqual(r.outcome,RecoveryOutcome.RETRIED_WAIT); self.assertEqual(client.calls,1)
            ...
            history=HistoryResult(BackendJobRef('i-ref2'),HistoryState.SUCCEEDED,{'outputs':{}}); backend3=Mock(); backend3.observe.return_value=history; coordinator=ChunkExecutionCoordinator(repo,SubmitAttemptUseCase(repo,client),history,extractor=X(),trusted_root=root,comfyui_output_root=root,correlator=lambda *_:corr,physical_validator=lambda *_:phys)
            ...
            out=ResumeExecutionUseCase(repo,backend3,coordinator,SubmitAttemptUseCase(repo,client), output_root=root).resume(p.id,e.id,prompt={}); self.assertEqual(out.outcome,RecoveryOutcome.RETRIED_COMPLETE); self.assertEqual(client.calls,1); repo.close()
