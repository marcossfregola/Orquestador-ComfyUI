...
class PersistenceTests(unittest.TestCase):
 ...
 def test_schema_fk(self):
  with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as d:
   self.r=r=SQLiteProjectRepository(d); self.assertEqual(r.db.execute('PRAGMA foreign_keys').fetchone()[0],1); self.assertEqual(r.db.execute('SELECT version FROM schema_version').fetchone()[0],6)
